...
export default function Statistics(){
 ...
 const ratedCorrect=scored.filter(a=>a.result==='correct'&&a.confidence); const guessedCorrect=ratedCorrect.filter(a=>a.confidence==='guessed').length;
 const guessRate=ratedCorrect.length?Math.round(guessedCorrect/ratedCorrect.length*100):null;
 ...
 const byTopic=useMemo(()=>{const m=new Map<string,{total:number;attempted:Set<string>;correct:number;scored:number;name:string;subject:string}>();for(const q of allQuestions){if(!m.has(q.topicId))m.set(q.topicId,{total:0,attempted:new Set(),correct:0,scored:0,name:q.topic,subject:q.subject});m.get(q.topicId)!.total++}for(const a of attempts){const q=qmap.get(a.question_id);if(!q)continue;const x=m.get(q.topicId);if(!x)continue;x.attempted.add(q.id);if(a.result==='correct')x.correct++;if(a.result==='correct'||a.result==='incorrect')x.scored++}return [...m.entries()].map(([id,x])=>({id,...x,accuracy:x.scored?Math.round(x.correct/x.scored*100):0})).filter(x=>x.attempted.size>0).sort((a,b)=>a.accuracy-b.accuracy)},[attempts,qmap]);
 ...
 <div className="grid stats-grid">
   ...
   <div className="card stat"><div className="label">Lucky guesses</div><div className="value">{guessRate!=null?`${guessRate}%`:'—'}</div><div className="muted" style={{fontSize:12,marginTop:4}}>{ratedCorrect.length?`of ${ratedCorrect.length} self-rated correct answers`:'Rate answers in Practice to unlock this'}</div></div>
 ...
 <div className="card section" style={{marginTop:16}}>
   <div className="section-head"><h3>Weakest topics</h3><span className="pill">{byTopic.length} attempted</span></div>
   {byTopic.length===0
     ? <div className="empty">Attempt some questions to see topic-level accuracy — this is the one level of detail subject accuracy alone can hide.</div>
     : byTopic.map(x=><div className="subject-row" key={x.id}><div><div className="subject-name">{x.name}</div><div className="subject-meta">{x.subject} · {x.attempted.size}/{x.total} attempted</div></div><span className="accuracy" style={{color:accuracyColor(x.accuracy)}}>{x.accuracy}%</span><div className="progress"><span style={{width:`${x.attempted.size/x.total*100}%`,background:accuracyColor(x.accuracy)}}/></div></div>)}
 </div></>;
}
